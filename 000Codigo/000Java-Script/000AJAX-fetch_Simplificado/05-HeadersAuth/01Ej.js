...
async function hacerEj2() {
  ...
  try {    
    ...
    const datos = await respuesta.json();
    console.log(datos); // Ya viene en json()???
  } catch (error) {
    console.error(error);
  }
...
async function fetchConHeaders(url,headersObj) {
  ...
  const respuesta = await fetch (urlCompleta,
    {
      headers:headersObj //{headers:{Accept:application}}
    });
  return await respuesta.json();
}
try {
  const datosP = await fetchConHeaders(urlCompleta,{'Accept':'application'}); //await tb
  console.log('Petion con función: ',datosP);
} 
catch (error) {
  console.error(error);
}
