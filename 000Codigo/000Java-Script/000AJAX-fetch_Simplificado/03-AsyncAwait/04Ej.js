...
async function obtenerDatos() {
  ...
  const fetchUsuario = await fetch('https://jsonplaceholder.typicode.com/users/1');
  ...
  /* Tu código aquí — reemplaza todo el bloque de TAREA 2 */
    const r = await fetch('https://jsonplaceholder.typicode.com/albums?userId=1');
    const albumes = await r.json();
       console.log('Albumes:', albumes);
  ...
  const fotos = await respFotos.json();
   ...
   console.table(fotos);
//PREGUNTA  QUE ES MEJOR CON try sin try
/*
  try{
   console.log('Fotos del álbum: "'+ primerAlbum.title + '": ',fotos );
   console.table(fotos);
