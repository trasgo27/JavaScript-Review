...
async function fetchConTimeout(url, timeout = 3000) {
    ...
    const signal = controlador.signal;        // ← Corregido: "señal" → "signal"
    //Programar Cancelación
    const tiempo = setTimeout(()=>
        controlador.abort(), timeout);
    try{
        const respuesta = await fetch(url,{signal});
        clearTimeout(tiempo);                  // ← Corregido: "timer" → "tiempo"
        return await respuesta.json();
    }
    catch (err){
        clearTimeout(tiempo);
        if(err.name === 'AbortError'){         // ← Corregido: "error" → "err" y nombre correcto
            console.log('Timeout: La petición tardo demasiado ...');
        }
        throw err;                             // ← Corregido: "error" → "err"
    }
}
...
hacerEj('TASK 1: fetchConTimeout', async () => {
    ...
    async function fetchConTimeout(url, timeout = 3000) {
        ...
        try {
            const resp = await fetch(url, { signal } );
            clearTimeout(timerID); //Borrar el timerID 
            return await resp.json();
        } catch (error) {
...
hacerEj('TASK 2: Probar timeout corto', async () => {
    ...
    try {
        ...
        const resultado = await fetchConTimeout('https://jsonplaceholder.typicode.com/users/1', 1); /* Corregido: URL completa, 1ms, con await */
...
hacerEj('TASK 3: fetchConReintentos', async () => {
    ...
    async function fetchConReintentos(url, maxReintentos = 3) {
        for (let contador = 1 ; contador <= maxReintentos ; contador++) {
            try {
                console.log('Intento ' + contador + '/' + maxReintentos + '...');
                const resp = await fetch(url);
                if (!resp.ok) throw new Error('HTTP ' + resp.status);
                return await resp.json();
            } catch (error) {
                console.log('Fallo en intento ' + contador + ':', error.message);
                if (contador === maxReintentos) throw error;
            }
        }
    }
...
hacerEj('TASK 4: Probar con URL invalida', async () => {
    ...
    try {
        ...
        const resultado = await fetchConReintentos('https://jsonplaceholder.typicode.com/users/99999') /* Corregido: solo URL, con await */
        console.log('Inesperado - funciono:', resultado);
    } catch (e) {
