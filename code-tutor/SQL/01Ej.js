...
console.log('\n=== Task 5: Handle UNIQUE violation ===');
try {
  insertUNO.run('Alice Dup', 'alice@example.com', 99);
} catch (err) {
...
const insertarUnP = db.prepare('INSERT INTO products(name,price,stock,category) VALUES(?,?,?,?)');
const insertarMuchosP = db.transaction((lista)=>{
  for (const p of lista) insertarUnP.run(...p)
});
insertarMuchosP(products);
...
const mouse = db.prepare('SELECT stock FROM products WHERE name = ?').get('Mouse');
